...
SRC=(seven_seg_dec_module.sv digit_sel_module.sv hr_cntr_module.sv min_cntr_module.sv sec_cntr_module.sv clock_module.sv tb.sv)
TRANSCRIPT=dofile
MODULE=tb
IFS=' '
...
for word in ${SRC[@]}
do
echo -e "\n \033[1;34mCompiling at RTL level: $word... \033[0m" | tee -a run.report
...
#Compiling Gates 
echo -e "\n \033[1;34mCompiling at Gate level... \033[0m" | tee -a run.report

dc_shell-xg-t -f dc_syn1  | tee -a run.report 
echo -e "\n\n____________________________________________________________________________\n\n"  >> run.report



if [ 1 -eq 1 ]; then #start comment
