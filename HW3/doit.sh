...
SRC=(updn_cntr.sv tb.sv )
TRANSCRIPT=dofile.do
MODULE=tb
IFS=' '
...
for word in ${SRC[@]}
do
echo -e "\n \033[1;34mCompiling $word... \033[0m"
vlog $word >> run.report 
...
if [ ! -f $TRANSCRIPT ]; then
   echo -e "\n \033[1;33mGenerating a transcript... \033[0m"
   echo -e "run 2000000\nquit" > $TRANSCRIPT
   echo -e "\n \033[1;34mStimulating ... \033[0m"
   vsim $MODULE -do $TRANSCRIPT -c -quiet -t 1ps >> run.report
else
   echo -e "\n \033[1;34mStimulating ... \033[0m"
   vsim $MODULE -do $TRANSCRIPT -c -quiet -t 1ps >> run.report
fi
...
#check if the Traylos's *.out has been imported
if [ ! -f updn_cntr.traylor.out ]; then
   #compiling modules from lib
   echo -e "\n\033[1;34mOnly this time Garrett. \nI'll get Traylor's output file for you... \033[0m"
   curl http://web.engr.oregonstate.edu/~traylor/ece474/12sp_hw/hw3/updn_cntr.out > updn_cntr.traylor.out
fi
